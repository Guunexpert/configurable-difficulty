...
public class SpawnModifierHandler {
    ...
    public static void reapplyNow(LivingEntity entity) {
        if (!BiomeDifficulty.getConfig().enabled) return;
        ...
        if (config.debugEnabled && config.debugLogBiomeChanges) {
            BiomeDifficulty.getLogger().info("Entity {} re-applied modifiers in biome {}", entity.getName(), biomeId);
        }
    }

    public static boolean shouldApplyToEntity(LivingEntity entity, BiomeConfig config) {
        // Players might be affected (luck only)
        if (entity instanceof Player) return true;

        // Mobs: check configuration
        if (entity instanceof Monster || entity instanceof org.bukkit.entity.Boss) return config.applyToHostileMobs;
