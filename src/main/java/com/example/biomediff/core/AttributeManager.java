...
public class AttributeManager {
    ...
    public static void applyModifiers(LivingEntity entity, String biomeId, BiomeMultipliers baseMultipliers) {
        ...
        // Combine all multipliers: dimension x biome x depth
        ...
        if (isPlayer) {
            if (enabled.luck && multipliers.luck != 1.0) {
                applyModifier(entity, Attribute.LUCK, LUCK_UUID,
        ...
        if (BiomeDifficulty.getConfig().debugEnabled && BiomeDifficulty.getConfig().debugLogAttributeChanges) {
            BiomeDifficulty.getLogger().info("Applied modifiers to {} - Dimension: {}, Biome: {}",
                entity.getName(), dimensionId, biomeId);
        }
    }
    ...
    public static void removeModifiers(LivingEntity entity) {
        ...
        // === MOB ATTRIBUTES ===
        if (!isPlayer) {
            if (enabled.maxHealth) removeModifier(entity, Attribute.MAX_HEALTH, MAX_HEALTH_UUID);
            if (enabled.armor) removeModifier(entity, Attribute.ARMOR, ARMOR_UUID);
            if (enabled.armorToughness) removeModifier(entity, Attribute.ARMOR_TOUGHNESS, ARMOR_TOUGHNESS_UUID);
            if (enabled.attackDamage) removeModifier(entity, Attribute.ATTACK_DAMAGE, ATTACK_DAMAGE_UUID);
            if (enabled.attackSpeed) removeModifier(entity, Attribute.ATTACK_SPEED, ATTACK_SPEED_UUID);
            if (enabled.attackKnockback) removeModifier(entity, Attribute.ATTACK_KNOCKBACK, ATTACK_KNOCKBACK_UUID);
            if (enabled.knockbackResistance) removeModifier(entity, Attribute.KNOCKBACK_RESISTANCE, KNOCKBACK_RESISTANCE_UUID);
        ...
        // === PLAYER ATTRIBUTE ===
        if (isPlayer) {
            if (enabled.luck) removeModifier(entity, Attribute.LUCK, LUCK_UUID);
        }
    }

    private static void applyModifier(LivingEntity entity, Attribute attribute, UUID uuid, String name, double value) {
        ...
        // Add new modifier using ADD_MULTIPLIED_BASE operation (equal to ADD_SCALAR)
    ...
    public static String getBiomeId(LivingEntity entity) {
        org.bukkit.block.Biome biome = entity.getWorld()
            .getBiome(entity.getLocation().getBlockX(), entity.getLocation().getBlockY(), entity.getLocation().getBlockZ());
        return biome.getKey().toString();
    }

    public static String getDimensionId(LivingEntity entity) {
        return entity.getWorld().getKey().toString();
    }
    ...
    private static BiomeMultipliers calculateDepthMultipliers(LivingEntity entity) {
        var config = BiomeDifficulty.getConfig().depthScaling;
        ...
        result.xp = lerp(1.0, config.maxMultipliers.xp, depthFactor);
    ...
    public static double getXpMultiplier(LivingEntity entity) {
        ...
        BiomeMultipliers combined = BiomeMultipliers.combine(dimensionMultipliers, biomeMultipliers);
        BiomeMultipliers total = BiomeMultipliers.combine(combined, depthMultipliers);
